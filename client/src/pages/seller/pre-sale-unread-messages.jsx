...
function PreSaleUnreadMessages() {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          if (response.data.messages.length > 0) {
            ...
            setMessage(null); // Handle the case where no messages are available
          }
        }
      } catch (err) {
  ...
  return (
    <div className="min-h-[80vh] my-10 mt-0 px-32">
      <h3 className="m-5 text-2xl font-semibold">Unread Pre-Sale Message</h3>
      <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
        {message ? (
          <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
              <tr>
                <th scope="col" className="px-6 py-3">Buyer ID</th>
                <th scope="col" className="px-6 py-3">Message</th>
                <th scope="col" className="px-6 py-3">Sender</th>
                <th scope="col" className="px-6 py-3">Received At</th>
                <th scope="col" className="px-6 py-3">View Message</th>
              </tr>
            </thead>
            <tbody>
              <tr className="bg-white dark:bg-gray-800 hover:bg-gray-50" key={message.id}>
                <td className="px-6 py-4">{message.senderId}</td>
                <td className="px-6 py-4">{message.text}</td>
                <td className="px-6 py-4">{message.sender.fullName || message.sender.username}</td>
