...
export const getSellerData = async (req, res, next) => {
  try {
    if (req.userId) {
      ...
      // New count for unread pre-transaction messages
      const preSaleUnreadMessages = await prisma.preTransactionMessage.count({
        where: {
          recipientId: req.userId,
          isRead: false,
        },
      ...
      return res.status(200).json({
        dashboardData: {
          ...
          gigs,
          unreadMessages,
          preSaleUnreadMessages,  // Include the new data in the response
          dailyRevenue,
          monthlyRevenue,
          revenue,
        },
